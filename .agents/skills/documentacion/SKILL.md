...
| `src/components/AgentChat.tsx` | ~658 | ✅ OK | Chat de Agente IA con selector HUD de modo (`🎯 Extractos Precisos` vs `📚 Toda la Base`), badges diferenciados de Cero Alucinación / Consulta Integral, acordeón de fuentes y preguntas dinámicas. |
...
| `app/llm/agent_service.py` | ~310 | ✅ OK | Orquestador RAG dual: modo `precise` (top-k similitud coseno) y modo `full` (análisis integral en bruto de todos los fragmentos del documento con prompt panorámico). |
